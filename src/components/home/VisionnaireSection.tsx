        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
              ...
              {luxuryImages.map((img, index) => (
                <div key={index} className={`absolute inset-0 w-full h-full transition-opacity duration-1000 ${index === currentImageIndex ? 'opacity-100' : 'opacity-0'}`}>
                  <img 
                    ...
                    loading="lazy"
                  />
                  <div className="absolute inset-0 bg-gradient-to-r from-[#1A1A1A]/50 to-transparent"></div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="text-white space-y-4 md:space-y-6 order-1 md:order-2">
            ...
            <p ref={textRef} className="text-white/80 text-base md:text-lg leading-relaxed">
              Our exclusive partnerships with Italian bespoke luxury brands brings the pinnacle of Italian luxury design to your doorstep. Each piece represents a harmonious blend of artistic vision and functional excellence, crafted with meticulous attention to detail and the finest materials.
            </p>
            <Link to="/collaboration" ref={ctaRef} className="inline-flex items-center gap-2 px-6 py-3 bg-[#C5A267] text-white font-medium rounded-sm hover:bg-[#D6B378] transition-colors duration-300 group min-h-[44px]">
